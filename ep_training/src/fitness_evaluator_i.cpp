...
FitnessEvaluatorI::FitnessEvaluatorI(const uint8_t id) :
    ...
    restart_ssv(create_service<std_srvs::srv::Trigger>(
        node_id.get_formatted_topic("restart_fitness_evaluator"),
        std::bind(
            &ep_training::FitnessEvaluatorI::handle_restart_requests,
            this,
            std::placeholders::_1,
            std::placeholders::_2
        )
    ...
    evaluate_solution_ssv(create_service<ep_common_interfaces::srv::GetDouble>(
        node_id.get_formatted_topic("evaluate_solution"),
        std::bind(
...
void FitnessEvaluatorI::handle_restart_requests(
